...
kstresults <- foreach(monk = monknames, .combine = "rbind", .errorhandling="remove") %do% {
...
rownames(kstresults) <- NULL
colnames(kstresults) <- c("monk", "week", "loc1", "loc2", "size1", "size2", "ds", "trial", "p")
kstout <- tbl_df(kstresults)
    
kstout$ds <-  as.numeric(kstout$ds)
kstout$p <-  as.numeric(kstout$p)
kstout <- mutate(kstout, p = 1-p)

write.table(kstout, "../out/kst/kstout.txt", row.names = FALSE, col.names = TRUE)
